...
def tournament_selection(population_list, fitness_score_list, elitism=True):
    new_species = []
    population_size = len(fitness_score_list)
    population_size = population_size - 1 if elitism else population_size
    for _ in range(0, population_size):
        of_parent_idx = random.randint(0, len(fitness_score_list) - 1)
        tf_parent_idx = random.randint(0, len(fitness_score_list) - 1)
        if fitness_score_list[of_parent_idx] > fitness_score_list[tf_parent_idx]:
            ch_winner = population_list[of_parent_idx]
        else:
            ch_winner = population_list[tf_parent_idx]
        new_species.append(ch_winner)
    new_species.append(population_list[np.argmax(fitness_score_list)])
    return new_species
...
for epoch in range(0, 100000):
    ...
    print("Epoch: {}, Population fitness score: {}, Max score: {}, Max chromosome: "
          "{}".format(epoch, 1. / np.mean(fit_scores), 1. / max_fitness, max_chromosome))
